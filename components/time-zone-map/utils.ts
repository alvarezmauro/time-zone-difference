import { rawTimeZones, type RawTimeZone as RawTimeZoneType } from "@vvo/tzdb";
...
export function convertOffsetInMinutesToString(
  ...
  return `${
    plusMinus + convertNumberToStringWithZeroPadding(hour)
...
export function findTimeZone(
  timeZoneName?: string,
): RawTimeZoneType | undefined {
  if (!timeZoneName) {
    return undefined;
  }

  const timeZone = rawTimeZones.find(
    (rawTimeZone) =>
      timeZoneName === rawTimeZone.name ||
      rawTimeZone.group.includes(timeZoneName),
  );
  return timeZone;
}
